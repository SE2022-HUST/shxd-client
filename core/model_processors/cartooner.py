...
def cartoonize(image, sigmaVal=50, minVal=100, maxVal=200):
    """
    ...
    # hists = []
    for i in range(c):
        output[:, :, i] = cv2.bilateralFilter(output[:, :, i], 5, sigmaVal, sigmaVal)
        # hist, _ = np.histogram(output[:, :, i], bins=np.arange(256+1))
        # hists.append(hist)
    # edge = cv2.Canny(output, 100, 200)
    # minVal, maxVal = 80, 150
    edge = cv2.Canny(output, minVal, maxVal)
